...
// go fmt doesn't like this being vertically aligned AND avoiding octal madness :'-(
var grid = [20][20]int{
...
func product(x, y, xDelta, yDelta int) int {
	ret := 1

	for i := 0; i < step && x > 0 && x < len(grid) && y > 0 && y < len(grid); i++ {
		val := grid[y][x]
...
func horizontal(x, y int) int {
	return product(x, y, 1, 0)
}

func vertical(x, y int) int {
	return product(x, y, 0, 1)
}

func diagonalDown(x, y int) int {
	return product(x, y, 1, 1)
}

func diagonalUp(x, y int) int {
	return product(x, y, 1, -1)
}
...
func main() {
	...
	// O(n^2), but n == 20 so let's just brute force it.
	for x := 0; x < len(grid)-step+1; x++ {
		for y := 0; y < len(grid)-step+1; y++ {
			best = max(best,
				horizontal(x, y),
				vertical(x, y),
				diagonalUp(x, y),
